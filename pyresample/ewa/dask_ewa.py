...
def _flatten_ll2cr_delayeds(ll2cr_delayeds, num_row_blocks, num_col_blocks):
    """Return ll2cr delayed blocks as a flat index-aware sequence.

    Args:
        ll2cr_delayeds: Nested list/array of delayed ll2cr blocks indexed by
            row/column block position.
        num_row_blocks: Number of ll2cr row blocks.
        num_col_blocks: Number of ll2cr column blocks.

    Returns:
        List of ``(row_index, col_index, delayed_block)`` tuples in row-major
        order.
    """
    return [
        (in_row_idx, in_col_idx, ll2cr_delayeds[in_row_idx][in_col_idx])
...
class DaskEWAResampler(BaseResampler):
    ...
    def _fill_block_cache_with_ll2cr_results(self, ll2cr_result,
        ...
        flat_delayeds = _flatten_ll2cr_delayeds(
            ll2cr_delayeds, num_row_blocks, num_col_blocks)
        block_cache = {}
        block_extents = None
        ...
        if persist:
            ...
            persisted_results = dask.compute(*persisted_delayeds)
            for (in_row_idx, in_col_idx, _), persisted_delayed, result in zip(
                    flat_delayeds, persisted_delayeds, persisted_results, strict=True):
                key = (ll2cr_result.name, in_row_idx, in_col_idx)
                # Skip empty input chunks completely in persisted mode.
                if isinstance(result[0], tuple):
                    continue
                block_cache[key] = persisted_delayed.key
                block_extents[key] = _ll2cr_block_extent(result)
    ...
    def _run_fornav_single(self, data, out_chunks, target_geo_def, fill_value, **kwargs):
        ...
        ll2cr_blocks = list(self.cache['ll2cr_blocks'].items())
